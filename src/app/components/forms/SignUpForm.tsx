...
const fullSchema = yup.object().shape({
    ...
    password: yup.string().when('$currentStep', {
        ...
        then: (schema) => schema.required('비밀번호를 입력해주세요.').min(8, '비밀번호는 8자 이상이어야 합니다.'),
        otherwise: (schema) => schema.notRequired(),
    }),
    confirmPassword: yup.string().when('$currentStep', {
        is: (step: number) => step >= 2,
        then: (schema) => schema.required('비밀번호 확인을 입력해주세요.').oneOf([yup.ref('password')], '비밀번호가 일치하지 않습니다.'),
        otherwise: (schema) => schema.notRequired(),
    }),
    gender: yup.string().when('$currentStep', {
        is: (step: number) => step >= 3,
        then: (schema) => schema.required('성별을 선택해주세요.').oneOf(['male', 'female'], '올바른 성별을 선택해주세요.'),
        otherwise: (schema) => schema.notRequired(),
    }),
    height: yup.number().when('$currentStep', {
        is: (step: number) => step >= 4,
        then: (schema) => schema.typeError('숫자만 입력 가능합니다.').required('키를 입력해주세요.').positive('키는 0보다 커야합니다.'),
        otherwise: (schema) => schema.notRequired(),
    }),
    weight: yup.number().when('$currentStep', {
        is: (step: number) => step >= 4,
        then: (schema) => schema.typeError('숫자만 입력 가능합니다.').required('몸무게를 입력해주세요.').positive('몸무게는 0보다 커야합니다.'),
        otherwise: (schema) => schema.notRequired(),
    }),
    nickname: yup.string().when('$currentStep', {
        is: (step: number) => step >= 5,
        then: (schema) => schema.required('닉네임을 입력해주세요.').min(3, '닉네임은 3자 이상이어야 합니다.'),
        otherwise: (schema) => schema.notRequired(),
    }),
}).required();
...
export default function SignUpForm() {
    ...
    // 에러 처리
    const onError: (errors: FieldErrors<SignUpFormData>) => void = (errorFields) => {
    ...
    const handleCheckUserId = async () => {
        ...
        try {
            ...
            // ✅ 수정: response.data?.available → response.available
    ...
    const handleNextStep = async () => {
        ...
        case 1:
            ...
            if (!isUserIdChecked) {
                setError('userId', { type: 'manual', message: '아이디 중복 확인을 해주세요.' });
                return;
            }
                setCurrentStep((prev) => prev + 1);
                break;
        case 2:
            const isStep2Valid = await trigger(['password', 'confirmPassword']);
            if (isStep2Valid) {
                setCurrentStep((prev) => prev + 1);
            }
                break;
    ...
    return (
        ...
        <div className="max-w-lg w-full px-4">
            ...
            <div className="card w-full mb-6">
                <h2 className="text-xl font-bold text-center mb-6 text-gray-800 break-keep 
                            sm:text-2xl">
                ...
                <div className="space-y-4">
                    ...
                    <input
                        ...
                        className={`w-full p-4 pr-24 border-2 rounded-xl text-base font-medium 
                                ...
                                ${errors.userId ? 'border-error' : 'border-gray-200 focus:border-primary'}`}
                    ...
                    {errors.userId && (
                        <p className="text-error text-sm font-medium">{errors.userId.message}</p>
                    )}
                    {isUserIdChecked && !errors.userId && (
                        <p className="text-success text-sm font-medium">사용 가능한 아이디입니다.</p>
                    )}
                </div>
                )}
                ...
                <div className="space-y-4">
                    ...
                    <input
                        ...
                        className={`w-full p-4 border-2 rounded-xl text-base font-medium 
                                transition-colors duration-200 focus:outline-none
                                ${errors.password ? 'border-error' : 'border-gray-200 focus:border-primary'}`}
                        placeholder="비밀번호 (8자 이상)"
                    />
                    {errors.password && (
                        <p className="text-error text-sm font-medium mt-2">{errors.password.message}</p>
                    )}
                    </div>
                    <div>
                    <input
                    ...
                    {errors.confirmPassword && (
                        <p className="text-error text-sm font-medium mt-2">{errors.confirmPassword.message}</p>
                    )}
                    </div>
                </div>
                )}
                ...
                <div className="space-y-4">
                    ...
                    <label className="flex flex-col items-center gap-3 p-5 border-2 border-gray-200 
                        ...
                        <span className="font-semibold text-gray-800">남성</span>
                    </label>
                    ...
                    <label className="flex flex-col items-center gap-3 p-5 border-2 border-gray-200 
                        ...
                        />
                        <div className="w-10 h-10 text-2xl">👩</div>
                        <span className="font-semibold text-gray-800">여성</span>
                    </label>
                    </div>
                    {errors.gender && (
                    <p className="text-error text-sm font-medium text-center">{errors.gender.message}</p>
                    )}
                </div>
                )}
                ...
                <div className="space-y-4">
                    ...
                    <div>
                        <label className="block text-sm font-semibold text-gray-800 mb-2">키 (cm)</label>
                        <input
                        id="height"
                        type="number"
                        ...
                        className={`w-full p-4 border-2 rounded-xl text-base font-medium 
                                    transition-colors duration-200 focus:outline-none
                                    ${errors.height ? 'border-error' : 'border-gray-200 focus:border-primary'}`}
                        placeholder="170"
                        />
                        {errors.height && (
                        <p className="text-error text-sm font-medium mt-1">{errors.height.message}</p>
                        )}
                    </div>
                    <div>
                        <label className="block text-sm font-semibold text-gray-800 mb-2">몸무게 (kg)</label>
                        <input
                        id="weight"
                        type="number"
                        ...
                        className={`w-full p-4 border-2 rounded-xl text-base font-medium 
                                    transition-colors duration-200 focus:outline-none
                                    ${errors.weight ? 'border-error' : 'border-gray-200 focus:border-primary'}`}
                        placeholder="70"
                        />
                        {errors.weight && (
                        <p className="text-error text-sm font-medium mt-1">{errors.weight.message}</p>
                        )}
                    </div>
                    </div>
                </div>
                ...
                <div className="space-y-4">
                    ...
                    className={`w-full p-4 border-2 rounded-xl text-base font-medium 
                                transition-colors duration-200 focus:outline-none
                                ${errors.nickname ? 'border-error' : 'border-gray-200 focus:border-primary'}`}
                    placeholder="3~10자리 입력"
                    />
                    {errors.nickname && (
                    <p className="text-error text-sm font-medium">{errors.nickname.message}</p>
                    )}
                </div>
                )}
            </div>
