...
export async function POST(request: NextRequest) {
    try {
        ...
        const userProfile = {
            gender: user.gender,
            height: user.addInfo?.height,
            weight: user.addInfo?.weight,
            disease: user.addInfo?.disease
        }
        ...
        if (!aiResponse) {
            ...
            return NextResponse.json(
                { success: false, error: 'AI 응답을 받지 못했습니다. 다시 시도해주세요.' },
                { status: 500 }
            )
        }
        ...
        try {
            ...
            return NextResponse.json(
                { success: false, error: 'AI 응답을 처리하는 중 오류가 발생했습니다. 다시 시도해주세요.' },
                { status: 500 }
            )
        }
        ...
        if (saveToDatabase && parsedWorkout.weeklyWorkout) {
            try {
                ...
                const workoutSavePromises = parsedWorkout.weeklyWorkout.map(async (dayWorkout: any, index: number) => {
                    const date = new Date()
        ...
        return NextResponse.json({
            ...
            data: {
                ...
                userProfile: {
                    ...
                    hasPhysicalInfo: !!(userProfile.height && userProfile.weight),
        ...
        if (error.code === 'context_length_exceeded') {
            return NextResponse.json(
                { success: false, error: '요청이 너무 깁니다. 더 간단하게 입력해주세요.' },
                { status: 400 }
            )
        }
        ...
        if (error.message?.includes('network') || error.message?.includes('timeout')) {
            return NextResponse.json(
                { success: false, error: '네트워크 오류가 발생했습니다. 다시 시도해주세요.' },
                { status: 503 }
            )
        }

        return NextResponse.json(
            { success: false, error: '운동 계획 생성 중 오류가 발생했습니다. 다시 시도해주세요.' },
            { status: 500 }
        )
    }
}